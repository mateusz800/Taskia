...
fun TaskList(viewModel: TaskListViewModel) {
    ...
    if (taskHashMap.value != null) {
        TaskList(taskMap = taskHashMap.value!!)
    }
}

@Composable
private fun TaskList(taskMap: Map<Task, List<Task>>) {
    LazyColumn {
        items(taskMap.keys.toList()) { task ->
            TaskItem(task = task, subtasks = taskMap[task])
        }
    }
}
...
private fun TaskList_Preview() {
    ...
    MaterialTheme {
        TaskList(taskMap = taskMap.toSortedMap(compareBy { it.order }))
    }
}