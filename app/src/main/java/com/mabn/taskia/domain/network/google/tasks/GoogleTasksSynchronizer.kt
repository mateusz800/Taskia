...
class GoogleTasksSynchronizer @Inject constructor(
    ...
    private var addToQueue: (task: Task, operation: SyncDataOperation) -> Unit = { _, _ -> }

    fun sync(
        account: ConnectedAccount,
        addToQueue: (task: Task, operation: SyncDataOperation) -> Unit
    ) {
    ...
    suspend fun updateGoogleTask(
        account: ConnectedAccount,
        task: Task,
        listId: String = task.googleTaskList ?: "",
        taskId: String = task.googleId ?: "",
    ) {
        ...
        try {
            ...
            if (response.code() == 401) {
                refreshGoogleToken(
                    account = account,
                    nextAction = { acc -> updateGoogleTask(acc, task, listId, taskId) })
            }
        } catch (error: UnknownHostException) {
            addToQueue(task, SyncDataOperation.UPDATE)
        }
    }
    ...
    private suspend fun refreshGoogleToken(
        ...
        try {
            ...
            return ConnectedAccount(type = AccountType.UNKNOWN, userIdentifier = "")
        }
    }
    ...
    suspend fun deleteGoogleTask(task: Task): Boolean {
        ...
        if (task.googleTaskList != null && task.googleId != null && account != null) {
            try {
                ...
                addToQueue(task, SyncDataOperation.DELETE)
            }
        }
        return false
    }
    ...
    suspend fun insertTask(task: Task) {
        ...
        if (task.googleTaskList != null && task.googleId != null && account != null) {
            try {
                ...
                addToQueue(task, SyncDataOperation.INSERT)
            }
        }
    }
}